...
interface GenericEnterprisePageProps {
  title: string;
  description: string;
  actionLabel?: string;
  columns?: string[];
...
export default function GenericEnterprisePage({ 
  title, 
  description, 
  actionLabel = "Add Record",
  columns = ["ID", "Name", "Type", "Status", "Created At"]
}: GenericEnterprisePageProps) {
  return (
        ...
        <div className="overflow-x-auto">
          <table className="w-full text-left text-xs whitespace-nowrap">
            ...
            <tbody className="divide-y divide-slate-700/40">
              {[1, 2, 3, 4, 5].map((item) => (
                <tr key={item} className="hover:bg-slate-800/50 transition-colors">
                  <td className="px-6 py-4 font-mono text-xs text-slate-400">REQ-{item}892</td>
                  <td className="px-6 py-4 font-bold text-slate-200">Enterprise Record {item}</td>
                  <td className="px-6 py-4 text-slate-300">Standard Configuration</td>
                  <td className="px-6 py-4">
                    <span className="inline-flex items-center gap-1.5 px-2.5 py-1 rounded-md text-[11px] font-semibold bg-blue-950/60 text-blue-400 border border-blue-800/60">
                      Processing
                    </span>
                  </td>
                  <td className="px-6 py-4 text-slate-400 font-mono">2026-07-0{item}</td>
