...
export default function CustomerHealthPage() {
  const kpis = [
    ...
    { title: "At Risk", value: "3" , trend: { value: "Requires investigation", isPositive: false, isNeutral: true } }
  ];

  const tableColumns = [
    { header: "Tenant", accessor: (row: any) => <span className="font-medium">{row.name || row.tenant || "Data"}</span> },
    { header: "Issues", accessor: (row: any) => <span className="font-medium">{row.issues || row.issues || "Data"}</span> }
  ];

  const tableData = [
    {"name":"TechFlow Inc","issues":"Packet Loss"},
    {"name":"City Telecom","issues":"Critical Alarms"}
  ];
