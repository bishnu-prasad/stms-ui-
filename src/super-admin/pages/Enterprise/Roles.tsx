...
export default function RolesPage() {
  const kpis = (
    <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-5">
      <KPICard title="Active Roles" value="8 Definitions" status="Root, Admin, NOC, Audit" delay={0.05} />
      <KPICard title="User Assignments" value="24 Accounts" status="Active users" delay={0.1} />
      <KPICard title="Audit Status" value="Cleared" status="Quarterly review" delay={0.15} />
      <KPICard title="Policy Compliance" value="100%" status="Zero over-privileged accounts" delay={0.2} />
    </div>
  );

  const charts = (
    <div className="grid grid-cols-1 lg:grid-cols-2 gap-6">
      <ChartCard title="Role Distribution across Users" subtitle="How are administrator accounts distributed across operational roles?" delay={0.25}>
  ...
  const recommendations = [
    {
      title: "Optimize Administrative Security Roles Manager Operational Parameters",
