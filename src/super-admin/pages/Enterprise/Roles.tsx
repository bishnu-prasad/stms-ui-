...
export default function RolesPage() {
  const kpis = [
    { title: "Total Roles", value: "24"  },
    { title: "Custom Roles", value: "8"  }
  ];

  const tableColumns = [
    { header: "Role Name", accessor: (row: any) => <span className="font-medium">{row.name || row.rolename || "Data"}</span> },
    { header: "Assigned Users", accessor: (row: any) => <span className="font-medium">{row.users || row.assignedusers || "Data"}</span> }
  ];

  const tableData = [
    {"name":"Super Admin","users":"12"},
    {"name":"Tenant Admin","users":"248"}
  ];
