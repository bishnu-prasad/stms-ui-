...
export default function DatabasePage() {
  const kpis = [
    { title: "DB Load", value: "24%"  },
    { title: "Replication Lag", value: "<10ms"  }
  ];

  const tableColumns = [
    { header: "Cluster", accessor: (row: any) => <span className="font-medium">{row.name || row.cluster || "Data"}</span> },
    { header: "Engine", accessor: (row: any) => <span className="font-medium">{row.engine || row.engine || "Data"}</span> },
    { header: "Status", accessor: (row: any) => <span className="font-medium">{row.status || row.status || "Data"}</span> }
  ];

  const tableData = [
    {"name":"stms-main-db","engine":"PostgreSQL 15","status":"Primary (Healthy)"}
  ];
