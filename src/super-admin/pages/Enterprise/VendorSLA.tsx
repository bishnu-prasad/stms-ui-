...
export default function VendorSLAPage() {
  const kpis = [
    ...
    { title: "SLA Breaches", value: "0"  }
  ];

  const tableColumns = [
    { header: "Vendor", accessor: (row: any) => <span className="font-medium">{row.name || row.vendor || "Data"}</span> },
    { header: "Uptime (30d)", accessor: (row: any) => <span className="font-medium">{row.uptime || row.uptime30d || "Data"}</span> },
    { header: "Support SLA", accessor: (row: any) => <span className="font-medium">{row.support || row.supportsla || "Data"}</span> }
  ];

  const tableData = [
    {"name":"Juniper","uptime":"100%","support":"Met (2h avg)"}
  ];
