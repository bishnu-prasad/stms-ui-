...
export default function ServersPage() {
  const kpis = [
    { title: "Total Nodes", value: "48"  },
    { title: "Avg CPU", value: "42%"  }
  ];

  const tableColumns = [
    { header: "Instance ID", accessor: (row: any) => <span className="font-medium">{row.id || row.instanceid || "Data"}</span> },
    { header: "Region", accessor: (row: any) => <span className="font-medium">{row.region || row.region || "Data"}</span> },
    { header: "CPU", accessor: (row: any) => <span className="font-medium">{row.cpu || row.cpu || "Data"}</span> }
  ];

  const tableData = [
    {"id":"i-0abcd1234","region":"us-east-1a","cpu":"45%"}
  ];
