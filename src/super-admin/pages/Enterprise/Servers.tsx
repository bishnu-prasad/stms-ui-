...
export default function ServersPage() {
  const kpis = (
    <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-5">
      <KPICard title="Active Instances" value="48 Nodes" status="Across 3 Availability Zones" delay={0.05} />
      <KPICard title="Fleet CPU Load" value="24.5%" status="Optimal range" delay={0.1} />
      <KPICard title="Disk I/O Latency" value="1.2 ms" status="NVMe SSD backed" delay={0.15} />
      <KPICard title="Auto-Scale Buffer" value="12 Nodes" status="Standby capacity" delay={0.2} />
    </div>
  );

  const charts = (
    <div className="grid grid-cols-1 lg:grid-cols-2 gap-6">
      <ChartCard title="Compute Instance CPU Utilization" subtitle="What is the CPU utilization distribution across availability zones?" delay={0.25}>
  ...
  const recommendations = [
    {
      title: "Optimize Cloud Compute Nodes & Server Fleet Operational Parameters",
