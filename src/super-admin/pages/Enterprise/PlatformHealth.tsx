...
export default function PlatformHealthPage() {
  const kpis = (
    <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-5">
      <KPICard title="Cluster Availability" value="99.98%" status="Exceeding 99.9% target" delay={0.05} />
      <KPICard title="Avg Gateway Latency" value="4.2 ms" status="Sub-10ms performance" delay={0.1} />
      <KPICard title="CPU Load Factor" value="22% Avg" status="Peak load 34%" delay={0.15} />
      <KPICard title="Memory Allocation" value="38% RAM" status="Buffer headroom 62%" delay={0.2} />
    </div>
  );

  const charts = (
    <div className="grid grid-cols-1 lg:grid-cols-2 gap-6">
      <ChartCard title="Microservice Latency & Throughput" subtitle="Are cloud microservices responding under sub-10ms SLAs during peak ingestion?" delay={0.25}>
  ...
  const recommendations = [
    {
      title: "Optimize Cloud Infrastructure & Service Health Operational Parameters",
