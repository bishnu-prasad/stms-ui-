...
export default function AlertsPage() {
  const kpis = [
    ...
    { title: "Warnings", value: "14"  }
  ];

  const tableColumns = [
    { header: "Severity", accessor: (row: any) => <span className="font-medium">{row.severity || row.severity || "Data"}</span> },
    { header: "Message", accessor: (row: any) => <span className="font-medium">{row.message || row.message || "Data"}</span> },
    { header: "Time", accessor: (row: any) => <span className="font-medium">{row.time || row.time || "Data"}</span> }
  ];

  const tableData = [
    {"severity":"Warning","message":"Edge Router CPU > 90%","time":"10 mins ago"}
  ];
