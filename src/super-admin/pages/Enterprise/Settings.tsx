...
export default function SettingsPage() {
  const kpis = (
    <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-5">
      <KPICard title="System Region" value="ap-south-1" status="Primary data center" delay={0.05} />
      <KPICard title="Security Level" value="High (SOC2)" status="MFA Mandatory" delay={0.1} />
      <KPICard title="API Rate Limit" value="500 req/s" status="Global ceiling" delay={0.15} />
      <KPICard title="Backup Window" value="02:00 UTC" status="Daily maintenance" delay={0.2} />
    </div>
  );

  const charts = (
    <div className="grid grid-cols-1 lg:grid-cols-2 gap-6">
      <ChartCard title="API Synchronization Latency" subtitle="Is real-time ticket synchronization responding under 100ms?" delay={0.25}>
  ...
  const recommendations = [
    {
      title: "Optimize Global Platform Settings & Configurations Operational Parameters",
