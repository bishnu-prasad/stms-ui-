...
export default function SettingsPage() {
  const kpis = [
    { title: "Config Sync", value: "100%"  },
    { title: "Active Regions", value: "3"  }
  ];

  const tableColumns = [
    { header: "Setting Group", accessor: (row: any) => <span className="font-medium">{row.group || row.settinggroup || "Data"}</span> },
    { header: "Last Modified", accessor: (row: any) => <span className="font-medium">{row.modified || row.lastmodified || "Data"}</span> },
    { header: "Modified By", accessor: (row: any) => <span className="font-medium">{row.by || row.modifiedby || "Data"}</span> }
  ];

  const tableData = [
    {"group":"SMTP / Email","modified":"Yesterday","by":"System Admin"},
    {"group":"SSO / SAML","modified":"3 days ago","by":"Security Team"}
  ];
