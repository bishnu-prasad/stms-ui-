...
export default function BackupPage() {
  ...
  const tableColumns = [
    { header: "Job Name", accessor: (row: any) => <span className="font-medium">{row.name || row.jobname || "Data"}</span> },
    { header: "Schedule", accessor: (row: any) => <span className="font-medium">{row.schedule || row.schedule || "Data"}</span> },
    { header: "Last Run", accessor: (row: any) => <span className="font-medium">{row.lastRun || row.lastrun || "Data"}</span> }
  ];

  const tableData = [
    {"name":"Postgres Full Dump","schedule":"Daily @ 02:00","lastRun":"Success (2h 14m)"}
  ];
