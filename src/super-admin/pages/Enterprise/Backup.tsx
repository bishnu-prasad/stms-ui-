...
export default function BackupPage() {
  const kpis = (
    <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-5">
      <KPICard title="Backup Status" value="100% Success" status="Zero failed jobs" delay={0.05} />
      <KPICard title="RPO Guarantee" value="< 15 Mins" status="Recovery Point Objective" delay={0.1} />
      <KPICard title="RTO Guarantee" value="< 1 Hour" status="Recovery Time Objective" delay={0.15} />
      <KPICard title="Last Snapshot" value="18.4 GB" status="Checksum verified" delay={0.2} />
    </div>
  );

  const charts = (
    <div className="grid grid-cols-1 lg:grid-cols-2 gap-6">
      <ChartCard title="Daily Backup Size & Transfer Speed" subtitle="Are automated database backup jobs completing within designated maintenance windows?" delay={0.25}>
  ...
  const recommendations = [
    {
      title: "Optimize Disaster Recovery & Backup Automation Operational Parameters",
