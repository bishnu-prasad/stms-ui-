...
export default function ReportsPage() {
  const kpis = [
    { title: "Generated (30d)", value: "1,420"  },
    { title: "Scheduled Jobs", value: "45"  }
  ];

  const tableColumns = [
    { header: "Report Name", accessor: (row: any) => <span className="font-medium">{row.name || row.reportname || "Data"}</span> },
    { header: "Type", accessor: (row: any) => <span className="font-medium">{row.type || row.type || "Data"}</span> },
    { header: "Generated", accessor: (row: any) => <span className="font-medium">{row.date || row.generated || "Data"}</span> }
  ];

  const tableData = [
    {"name":"Q3 Enterprise SLA","type":"Compliance","date":"Oct 1, 2026"}
  ];
