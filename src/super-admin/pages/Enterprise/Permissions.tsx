...
export default function PermissionsPage() {
  const kpis = [
    { title: "Total Policies", value: "142"  }
  ];

  const tableColumns = [
    { header: "Policy Name", accessor: (row: any) => <span className="font-medium">{row.name || row.policyname || "Data"}</span> },
    { header: "Scope", accessor: (row: any) => <span className="font-medium">{row.scope || row.scope || "Data"}</span> }
  ];

  const tableData = [
    {"name":"billing:read","scope":"Global"},
    {"name":"devices:write","scope":"Tenant"}
  ];
