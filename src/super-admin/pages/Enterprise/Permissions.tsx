...
export default function PermissionsPage() {
  const kpis = (
    <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-5">
      <KPICard title="System Roles" value="8 Roles" status="Pre-configured RBAC" delay={0.05} />
      <KPICard title="Granular Rights" value="64 Privileges" status="CRUD matrix" delay={0.1} />
      <KPICard title="Assigned Users" value="24 Admins" status="100% Audited" delay={0.15} />
      <KPICard title="Security Level" value="Strict" status="Zero-Trust Model" delay={0.2} />
    </div>
  );

  const charts = (
    <div className="grid grid-cols-1 lg:grid-cols-2 gap-6">
      <ChartCard title="Permission Assignment Distribution" subtitle="Which security roles carry administrative write privileges across microservices?" delay={0.25}>
  ...
  const recommendations = [
    {
      title: "Optimize Platform RBAC & Security Permissions Operational Parameters",
