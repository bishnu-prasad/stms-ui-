...
export default function VendorsPage() {
  const kpis = [
    { title: "Active Vendors", value: "14"  },
    { title: "Catalogs Updated", value: "2"  }
  ];

  const tableColumns = [
    { header: "Vendor Name", accessor: (row: any) => <span className="font-medium">{row.name || row.vendorname || "Data"}</span> },
    { header: "Supported Models", accessor: (row: any) => <span className="font-medium">{row.models || row.supportedmodels || "Data"}</span> },
    { header: "Integration", accessor: (row: any) => <span className="font-medium">{row.integration || row.integration || "Data"}</span> }
  ];

  const tableData = [
    {"name":"Cisco","models":"142","integration":"Healthy"}
  ];
