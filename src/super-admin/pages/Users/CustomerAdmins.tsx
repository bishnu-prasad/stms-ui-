...
import { Search, Plus, Filter, MoreHorizontal, CheckCircle2, XCircle } from "lucide-react";
  ...
  return (
    <div className="space-y-6">
      <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4">
        <div>
          <h1 className="text-2xl font-bold text-slate-100 tracking-tight">Customer Administrators</h1>
          <p className="text-xs text-slate-400 mt-1">Manage tenant-level administrative users.</p>
        </div>
      ...
      <div className="bg-[#1E293B] border border-slate-700/60 rounded-xl shadow-md overflow-hidden">
        <div className="p-4 border-b border-slate-700/60 flex items-center justify-between bg-slate-900/40">
          <div className="relative w-72">
            ...
            <input
              type="text"
              placeholder="Search customer admins..."
              value={searchTerm}
            ...
            <tbody className="divide-y divide-slate-700/40">
              {filtered.map((user) => (
                <tr key={user.id} className="hover:bg-slate-800/50 transition-colors">
                  ...
                  <td className="px-6 py-4 text-slate-200 font-medium">Reliance Jio (Example)</td>
