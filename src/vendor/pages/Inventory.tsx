...
export default function InventoryPage() {
  ...
  const kpis = (
    <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-5">
      <VendorMetricCard title="Available Parts" value="93 Units" subtitle="Across 5 Categories" badgeText="" icon={<Wrench className="w-5 h-5 text-teal-600" />} delay={0.05} />
      <VendorMetricCard title="Reserved for Jobs" value="25 Units" subtitle="Active work orders" icon={<Clock className="w-5 h-5 text-teal-600" />} delay={0.1} />
      <VendorMetricCard title="In Transit" value="51 Units" subtitle="Dispatched from warehouse" badgeText="" icon={<UserCheck className="w-5 h-5 text-teal-600" />} delay={0.15} />
      <VendorMetricCard title="Inventory Value" value="₹34.8L" subtitle="Field spare parts asset" icon={<Boxes className="w-5 h-5 text-teal-600" />} delay={0.2} />
    </div>
  );

  const charts = (
    <div className="grid grid-cols-1 lg:grid-cols-2 gap-6">
      <VendorChartCard title="Spare Parts Consumption Breakdown" businessQuestion="Which component replacement incurs the highest material consumption?" delay={0.25}>
