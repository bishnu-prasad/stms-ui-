...
export default function NotificationsPage() {
  ...
  const kpis = (
    <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-5">
      <VendorMetricCard title="Alerts Sent" value="48" subtitle="Today via SMS/App" badgeText="" icon={<Wrench className="w-5 h-5 text-teal-600" />} delay={0.05} />
      <VendorMetricCard title="Customer Reviews" value="12 New" subtitle="100% Positive" icon={<Clock className="w-5 h-5 text-teal-600" />} delay={0.1} />
      <VendorMetricCard title="Critical Dispatches" value="3" subtitle="Under 15m response" badgeText="" icon={<UserCheck className="w-5 h-5 text-teal-600" />} delay={0.15} />
      <VendorMetricCard title="Delivery Success" value="99.8%" subtitle="Push notifications" icon={<Boxes className="w-5 h-5 text-teal-600" />} delay={0.2} />
    </div>
  );

  const charts = (
    <div className="grid grid-cols-1 lg:grid-cols-2 gap-6">
      <VendorChartCard title="Notification Dispatch Channels" businessQuestion="How quickly are field engineers acknowledging dispatch alerts?" delay={0.25}>
