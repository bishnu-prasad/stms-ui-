...
export default function AssignedSitesPage() {
  ...
  const kpis = (
    <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-5">
      <VendorMetricCard title="Contracted Sites" value="4 Sites" subtitle="Across 3 Circles" badgeText="" icon={<Wrench className="w-5 h-5 text-teal-600" />} delay={0.05} />
      <VendorMetricCard title="Avg Site Health" value="80.0%" subtitle="1 Site in Critical state" icon={<Clock className="w-5 h-5 text-teal-600" />} delay={0.1} />
      <VendorMetricCard title="Equipment Assets" value="16 Assets" subtitle="SMPS, Battery Banks, OPE" badgeText="" icon={<UserCheck className="w-5 h-5 text-teal-600" />} delay={0.15} />
      <VendorMetricCard title="Preventive Maintenance" value="100%" subtitle="Q2 PM routines completed" icon={<Boxes className="w-5 h-5 text-teal-600" />} delay={0.2} />
    </div>
  );

  const charts = (
    <div className="grid grid-cols-1 lg:grid-cols-2 gap-6">
      <VendorChartCard title="Site Power Source & Health Metrics" businessQuestion="What is the health and backup runtime profile across customer sites?" delay={0.25}>
