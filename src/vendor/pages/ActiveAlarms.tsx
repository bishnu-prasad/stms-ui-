...
export default function ActiveAlarmsPage() {
  ...
  const kpis = (
    <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-5">
      <VendorMetricCard title="Active Alarms" value="3 Alarms" subtitle="1 Critical, 2 Major" badgeText="" icon={<Wrench className="w-5 h-5 text-teal-600" />} delay={0.05} />
      <VendorMetricCard title="SLA Countdown" value="28 mins" subtitle="Nearest SLA expiry" icon={<Clock className="w-5 h-5 text-teal-600" />} delay={0.1} />
      <VendorMetricCard title="MTTR Average" value="1.2 hrs" subtitle="Mean Time to Repair" badgeText="" icon={<UserCheck className="w-5 h-5 text-teal-600" />} delay={0.15} />
      <VendorMetricCard title="Acknowledged Alarms" value="100%" subtitle="All alerts assigned" icon={<Boxes className="w-5 h-5 text-teal-600" />} delay={0.2} />
    </div>
  );

  const charts = (
    <div className="grid grid-cols-1 lg:grid-cols-2 gap-6">
      <VendorChartCard title="Alarm Frequency by Equipment Type" businessQuestion="Which hardware component component generates the most fault tickets?" delay={0.25}>
