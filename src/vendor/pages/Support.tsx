...
export default function SupportPage() {
  ...
  const kpis = (
    <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-5">
      <VendorMetricCard title="Open Support Tickets" value="0" subtitle="No outstanding tickets" badgeText="" icon={<Wrench className="w-5 h-5 text-teal-600" />} delay={0.05} />
      <VendorMetricCard title="Avg Helpdesk Speed" value="4 mins" subtitle="Response time" icon={<Clock className="w-5 h-5 text-teal-600" />} delay={0.1} />
      <VendorMetricCard title="System Uptime" value="99.99%" subtitle="STMS Sync API" badgeText="" icon={<UserCheck className="w-5 h-5 text-teal-600" />} delay={0.15} />
      <VendorMetricCard title="Emergency Backup" value="Ready" subtitle="L3 Expert Desk" icon={<Boxes className="w-5 h-5 text-teal-600" />} delay={0.2} />
    </div>
  );

  const charts = (
    <div className="grid grid-cols-1 lg:grid-cols-2 gap-6">
      <VendorChartCard title="Support Ticket Resolution Efficiency" businessQuestion="How quickly does STMS L3 support resolve complex hardware diagnostic requests?" delay={0.25}>
