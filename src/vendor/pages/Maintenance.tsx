...
export default function MaintenancePage() {
  ...
  const kpis = (
    <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-5">
      <VendorMetricCard title="Upcoming PMs" value="3 Audits" subtitle="Scheduled for July" badgeText="" icon={<Wrench className="w-5 h-5 text-teal-600" />} delay={0.05} />
      <VendorMetricCard title="Completed PMs" value="1 Audit" subtitle="100% Sign-off" icon={<Clock className="w-5 h-5 text-teal-600" />} delay={0.1} />
      <VendorMetricCard title="Missed PMs" value="0" subtitle="Zero SLA violations" badgeText="" icon={<UserCheck className="w-5 h-5 text-teal-600" />} delay={0.15} />
      <VendorMetricCard title="PM Compliance" value="100%" subtitle="On-time maintenance" icon={<Boxes className="w-5 h-5 text-teal-600" />} delay={0.2} />
    </div>
  );

  const charts = (
    <div className="grid grid-cols-1 lg:grid-cols-2 gap-6">
      <VendorChartCard title="Quarterly PM Completion vs Target" businessQuestion="Are field teams adhering to quarterly preventive maintenance quotas?" delay={0.25}>
