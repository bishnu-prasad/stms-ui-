...
export default function SettingsPage() {
  ...
  const kpis = (
    <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-5">
      <VendorMetricCard title="SLA Threshold" value="30 Mins" subtitle="Warning trigger" badgeText="" icon={<Wrench className="w-5 h-5 text-teal-600" />} delay={0.05} />
      <VendorMetricCard title="Auto-Dispatch" value="Enabled" subtitle="Proximity matching" icon={<Clock className="w-5 h-5 text-teal-600" />} delay={0.1} />
      <VendorMetricCard title="Max Dispatch Distance" value="50 km" subtitle="Per engineer" badgeText="" icon={<UserCheck className="w-5 h-5 text-teal-600" />} delay={0.15} />
      <VendorMetricCard title="API Gateway Status" value="Connected" subtitle="STMS Customer Sync" icon={<Boxes className="w-5 h-5 text-teal-600" />} delay={0.2} />
    </div>
  );

  const charts = (
    <div className="grid grid-cols-1 lg:grid-cols-2 gap-6">
      <VendorChartCard title="API Synchronization Latency" businessQuestion="Is real-time ticket synchronization responding under 100ms?" delay={0.25}>
