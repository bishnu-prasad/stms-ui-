...
export default function ReportsPage() {
  ...
  const kpis = (
    <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-5">
      <VendorMetricCard title="Generated Reports" value="142" subtitle="This month" badgeText="" icon={<Wrench className="w-5 h-5 text-teal-600" />} delay={0.05} />
      <VendorMetricCard title="Customer Exports" value="48 PDFs" subtitle="Sent for sign-off" icon={<Clock className="w-5 h-5 text-teal-600" />} delay={0.1} />
      <VendorMetricCard title="Audit Log Integrity" value="100%" subtitle="Cryptographically verified" badgeText="" icon={<UserCheck className="w-5 h-5 text-teal-600" />} delay={0.15} />
      <VendorMetricCard title="Billing Attachments" value="Ready" subtitle="SLA verification ready" icon={<Boxes className="w-5 h-5 text-teal-600" />} delay={0.2} />
    </div>
  );

  const charts = (
    <div className="grid grid-cols-1 lg:grid-cols-2 gap-6">
      <VendorChartCard title="Monthly Maintenance Report Generations" businessQuestion="What is the volume trend of service completion reports generated?" delay={0.25}>
